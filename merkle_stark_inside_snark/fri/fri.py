...
from polynomial import Polynomial, test_colinearity
from proofstream import ProofStream
...
class Fri:
    ...
    def commit(self, codeword, proof_stream, round_index = 0):
        ...
        for r in range(self.num_rounds()):
            ...
            root = MerkleTree.commit(codeword)
            ...
            codeword = [
                two.inverse() *
                    ((one + alpha / (offset * (omega ** i))) * codeword[i] + (one - alpha / (offset * (omega^i)) ) * codeword[len(codeword)//2 + i] ) 
    ...
    def query(self, current_codeword, next_codeword, c_indices, proof_stream):
        ...
        for s in range(self.num_colinearity_tests):
            proof_stream.push(MerkleTree.open(a_indices[s], current_codeword))
            proof_stream.push(MerkleTree.open(b_indices[s], current_codeword))
            proof_stream.push(MerkleTree.open(c_indices[s], next_codeword))
    ...
    def verify(self, proof_stream, polynomial_values):
        ...
        if roots[-1] != MerkleTree.commit(last_codeword):
        ...
        assert(last_omega.inverse() == last_omega ** (len(last_codeword) - 1)), "omega does not have the right order"
        ...
        for r in range(0, self.num_rounds() - 1):
            ...
            for i in range(self.num_colinearity_tests):
                ...
                if MerkleTree.verify(roots[r], a_indices[i], path, aa[i]) == False:
                    print("merkle authentication path verification fails for aa")
                    return False
                path = proof_stream.pull()
                if MerkleTree.verify(roots[r], b_indices[i], path, bb[i]) == False:
                    print("merkle authentication path verification fails for bb")
                    return False
                path = proof_stream.pull()
                if MerkleTree.verify(roots[r+1], c_indices[i], path, cc[i]) == False:
