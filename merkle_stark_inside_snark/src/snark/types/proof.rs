...
impl OpeningSetValues<Goldilocks, 2> {
    pub fn assign(
        ...
        let constants = opening_set_values
            .constants
            .iter()
            .map(|c| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, c))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
        let plonk_sigmas = opening_set_values
            .plonk_sigmas
            .iter()
            .map(|s| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, s))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
        let wires = opening_set_values
            .wires
            .iter()
            .map(|w| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, w))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
        let plonk_zs = opening_set_values
            .plonk_zs
            .iter()
            .map(|z| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, z))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
        let plonk_zs_next = opening_set_values
            .plonk_zs_next
            .iter()
            .map(|z_next| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, z_next))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
        let partial_products = opening_set_values
            .partial_products
            .iter()
            .map(|p| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, p))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
        let quotient_polys = opening_set_values
            .quotient_polys
            .iter()
            .map(|q| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, q))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
...
impl FriQueryStepValues<Goldilocks, 2> {
    pub fn assign(
        ...
        let evals = fri_query_step_values
            .evals
            .iter()
            .map(|v| ExtensionFieldValue::assign(verifier, ctx, main_gate_config, v))
            .collect::<Result<Vec<AssignedExtensionFieldValue<Goldilocks, 2>>, Error>>()?;
    ...
    pub fn assign(
        ...
        let merkle_proofs = fri_query_round_values
            ...
            .map(|(_, merkle_proof_values)| {
                MerkleProofValues::assign(verifier, ctx, main_gate_config, merkle_proof_values)
            })
        ...
        let steps = fri_query_round_values
            ...
            .map(|fri_query_step_values| {
                FriQueryStepValues::assign(verifier, ctx, main_gate_config, fri_query_step_values)
            })
    ...
    pub fn assign(
        ...
        let commit_phase_merkle_cap_values = fri_proof_values
            ...
            .map(|merkle_cap_values| {
                MerkleCapValues::assign(verifier, ctx, main_gate_config, merkle_cap_values)
            })
        ...
        let query_round_proofs = fri_proof_values
            ...
            .map(|fri_query_round_values| {
                FriQueryRoundValues::assign(verifier, ctx, main_gate_config, fri_query_round_values)
            })
...
impl ProofValues<Goldilocks, 2> {
    pub fn assign(
        ...
        let wires_cap = MerkleCapValues::assign(verifier, ctx, main_gate_config, &proof.wires_cap)?;
        ...
        let quotient_polys_cap =
            MerkleCapValues::assign(verifier, ctx, main_gate_config, &proof.quotient_polys_cap)?;
        let openings = OpeningSetValues::assign(verifier, ctx, main_gate_config, &proof.openings)?;
        let opening_proof =
            FriProofValues::assign(verifier, ctx, main_gate_config, &proof.opening_proof)?;
