...
use super::{matrix::Matrix, spec::MDSMatrix};
...
pub fn mds_matrix<const T: usize, const RATE: usize>() -> MDSMatrix<T, RATE> {
    let mds = MDS_MATRIX
        .to_vec()
        .iter()
        .map(|row| row.iter().map(|e| F::from_canonical_u64(*e)).collect())
        .collect();
    MDSMatrix(Matrix::from_vec(mds))
}

pub fn get_round_constants<const T: usize>() -> Vec<[F; T]> {
    ROUND_CONSTANTS
        .to_vec()
        .iter()
        .map(|l| {
            let mut round_constants = [F::from_canonical_u64(0); T];
            for (c, elem) in round_constants.iter_mut().zip(l.iter()) {
                *c = F::from_canonical_u64(*elem);
            }
            round_constants
        })
        .collect()
